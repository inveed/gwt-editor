...
public class JSEntity implements IJSObject {
	...
	public static final JSEntity EMPTY = new JSEntity();
	...
	public boolean isValid() {
		return this.entityManager != null && this.type != null;
	}
	...
	private String getDisplayValue(JSONObject entity) {
		List<IPropertyDescriptor<?>> nameFields = this.getModel().getNamePropertyDescriptors();
		if (nameFields != null && nameFields.size() > 0) {
			StringBuilder sb = new StringBuilder();
			for (IPropertyDescriptor<?> pd : this.getModel().getNamePropertyDescriptors()) {
	...
	void updateFromJson(JSONObject json) {
		...
		for (IPropertyDescriptor<?> prop : this.getModel().getAllPropertyDescriptors()) {
			try {
				JSONValue jv = prop.getJSONValue(json);
				if (jv.isNull() == null) {
